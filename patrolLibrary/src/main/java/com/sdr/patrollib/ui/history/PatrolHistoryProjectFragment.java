...
public class PatrolHistoryProjectFragment extends PatrolBaseFragment<PatrolHistoryProjectPresenter> implements PatrolHistoryProjectContract.View {
    ...
    private Map<Long, List<PatrolHistoryProject>> dataMap = new HashMap<>();
    private PatrolHistoryProjectRecyclerAdapter patrolHistoryProjectRecyclerAdapter;


    @Override
    protected PatrolHistoryProjectPresenter instancePresenter() {
        return new PatrolHistoryProjectPresenter();
    }
    ...
    protected void onFragmentFirstVisible() {
        patrolHistoryProjectRecyclerAdapter = new PatrolHistoryProjectRecyclerAdapter(R.layout.patrol_layout_item_recycler_history_project);
        patrolHistoryProjectRecyclerAdapter.setEmptyView(getEmptyView());
        recyclerView.setLayoutManager(new LinearLayoutManager(getContext()));
        recyclerView.setAdapter(patrolHistoryProjectRecyclerAdapter);
        // 设置起始时间
        ...
        // 获取数据
        showLoadingView();
        presenter.getHistoryProject(startTime, endTime);
        // calendar
        {
            // 展开
        ...
        // listener
        {
            ...
            patrolHistoryProjectRecyclerAdapter.setOnItemClickListener(onItemClickListener);
        }
    }
    ...
    private void refreshRecycler(long time) {
        // 找出集合
        List<PatrolHistoryProject> patrolHistoryMobiles = dataMap.get(time);
        patrolHistoryMobiles = (patrolHistoryMobiles == null ? new ArrayList<PatrolHistoryProject>() : patrolHistoryMobiles);
        patrolHistoryProjectRecyclerAdapter.setNewData(patrolHistoryMobiles);
    }
    ...
    @Override
    public void loadHistoryProjcetListSuccess(List<PatrolHistoryProject> patrolHistoryProjectList) {
        //
        dataMap.clear();
        List<Calendar> schemes = new ArrayList<>();
        for (PatrolHistoryProject historyMobile : patrolHistoryProjectList) {
            long patroltime = transfromDate(historyMobile.getPatrolStartTime());
            ...
            if (dataMap.containsKey(patroltime)) {
                ...
                List<PatrolHistoryProject> list = new ArrayList<>();
